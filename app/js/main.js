window.onload = function() {
  ...
  form.addEventListener(
    ...
    tabSearch,
    false
  ); /* Show Data when pressed TAB on search input field */
  ...
  function searchArtist() {
    ...
    const query = artist
      .replace(/\s/g, "+")
      .toLowerCase(); /* replace spaces with + sign to provide correct content request */
    return query;
  }
    ...
    xhr.onload = function() {
      if (this.status == 200 && this.readyState == 4) {
        ...
        if (outputData.length > 0) {
          /* If there are any results display them on page */
          loadData(outputData);
        } else {
...
Truncate title or artist strings on smallest screens */
  ...
  function loadData(output) {
        ...
        <section class="album__list">
            <div class="album__list__album--item">
                ...
                <div class="album__list__album__details">
                    <div class="album__list__details--artist">${
                      output[i].artistName
                    }</div>
                    <div class="album__list__details--title">${
                      output[i].collectionName
                    }</div>
                    <div class="album__list__details--offer">
                        <div class="album__list__details--price">
                            <span>Price:</span>${output[i].collectionPrice ||
                              "to be announced"} ${output[i].currency}
                        </div>
                        <a href="${
                          output[i].collectionViewUrl
                        }" target="_blank" class="album__list__album--shop">
                            <span>
    ...
    const artist = Array.from(
      document.querySelectorAll(".album__list__details--artist")
    );
    const title = Array.from(
      document.querySelectorAll(".album__list__details--title")
    );
    ...
    function WidthChange(mq) {
      if (mq.matches) {
        shave(
          ".album__list__details--artist",
          100
        ); /*_ Truncate strings in artist name and title in smallest resolution */
    ...
    function preventJump() {
      if (parent.classList.contains("search__bar--fixed")) {
        ...
        const diff =
          height -
          height2 +
          "px"; /* Set margin-top value for container element to prevent jump */
      ...
      function changeMarginXl(lq) {
        if (lq.matches) {
          document.body.querySelector(
            ".search__bar__wrapper--header--fixed"
          ).style.marginTop = "1rem";
          document.body.querySelector(
            ".search__bar__form--fixed"
          ).style.marginTop = "0";
          document.body.querySelector(
            ".search__bar__form--button--fixed"
  ...
  function calculateScrollHeight() {
    ...
    const scrollTop =
      window.pageYOffset; /* How much user scrolled page vertically */
    const docHeight = getDocumentHeight(); /* Document height */
    const heightDiff =
      docHeight -
      windowHeight; /* Difference between document height and browser height */
    const percentageScrolled = Math.floor(
      (scrollTop / heightDiff) * 100
